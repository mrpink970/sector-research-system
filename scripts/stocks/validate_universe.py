...
def validate_ticker(ticker: str) -> dict:
    ...
    try:
        ...
        # Convert to timezone-naive for comparison (fix timezone issue)
        if last_date.tzinfo is not None:
            last_date = last_date.tz_localize(None)
        
        # Check if data is recent (using naive datetime)
        now_naive = datetime.now()
        days_old = (now_naive - last_date).days
        ...
        if days_old > 5:
            status = "STALE_DATA"
            reason = f"No data since {last_date.strftime('%Y-%m-%d')} ({days_old} days ago)"
