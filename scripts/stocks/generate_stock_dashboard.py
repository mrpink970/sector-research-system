...
def load_trend_candidates_with_priority(scores_df: pd.DataFrame, date: str, min_score: int = 6) -> pd.DataFrame:
    """
    Load trend candidates with same priority sorting as trading script:
    ...
    candidates["sort_key_volume"] = candidates.get("volume_score", 0).fillna(0)
...
def load_breakout_candidates_with_priority(scores_df: pd.DataFrame, date: str, min_score: int = 6) -> pd.DataFrame:
    """
    Load breakout candidates with same priority sorting as trading script:
    ...
    candidates["sort_key_proximity"] = candidates.get("breakout_proximity_score", 0).fillna(0)
...
def generate_html(perf: Dict, positions: Dict, trades: pd.DataFrame, trend_candidates: pd.DataFrame, breakout_candidates: pd.DataFrame, run_log: Dict) -> str:
    ...
    # Build breakout candidates table
        ...
        <div class="two-col">
            <div class="col">
                ...
                <table style="width: 100%;">
                    ...
                    <tbody>
                        {build_trend_candidates_table(trend_candidates)}
                    </tbody>
                </table>
            </div>
            <div class="col">
                ...
                <table style="width: 100%;">
                    <thead><tr><th>Ticker</th><th>Score</th><th>Price</th></tr></thead>
                    <tbody>
                        {build_breakout_candidates_table(breakout_candidates)}
                    </tbody>
                </table>
            </div>
        </div>
