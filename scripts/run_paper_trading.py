...
subset = scores[(scores["sector"] == sector) & (scores["date"] <= signal_date)].sort_valu
...
day = day.sort_values(["strength", "sector"], ascending=[False, True]).reset_index(drop=T
return day
def close_position(
position: Position,
exit_date: str,
...
continue
current_gain_pct = (position.highest_price - position.entry_price) / position.ent
position.stop_pct = stepped_stop_pct_for_ticker(position.ticker, current_gain_pct
...
days_held = (pd.to_datetime(trade_date) - pd.to_datetime(position.entry_date)).da
...
current_gain_pct = (position.highest_price - position.entry_price) / position
position.stop_pct = stepped_stop_pct_for_ticker(position.ticker, current_gain
position.trailing_stop = position.highest_price * (1 - position.stop_pct)
survivors.append(position)
